...
class Alice(nn.Module):
    def __init__(self,config):
        ...
        self.LSTM1 = nn.LSTM(input_size=1,hidden_size=64,num_layers=2,batch_first=True)
        ...
        c0 = torch.zeros(2,config["batch_size"],64).to(self.device)
        self.hidden0=(h0,c0)
        h1 = torch.zeros(1,config["batch_size"],128).to(self.device)
        c1 = torch.zeros(1,config["batch_size"],128).to(self.device)
        self.hidden1=(h1,c1)
        
    def forward(self, x):
        # x = torch.cat((x, key), dim=1)
        x = self.p1(x)
        x, _ = self.LSTM1(x,self.hidden0)
        x, _ = self.LSTM2(x,self.hidden1)
        x = self.p2(x[:, -1, :])
        return x
